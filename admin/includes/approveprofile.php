...
<?php
	...
	if($_POST["actiune"]=="approve")
	{
		...
		foreach($_POST['approve'] as $key=>$value)
		{
			@mysql_query("UPDATE `tblUsers` SET `introtitle` = '" . addslashes($_POST["introtitle"][$key]) . "', 
			                                   ...
			                                   `approved` = 'Y', `approvedby` = '1', `approveddate` = NOW() WHERE `id` = '".$value."'");
		     ...
		     */
		    admin_send_mail($db, null,         'approved', 'internal', $value, 1);
		    admin_send_mail($db, 'emailnotif', 'approved', 'external', $value, 1);
		}
	}
	...
	if($_POST["actiune"]=="dissapprove")
	{
		...
		foreach($_POST['approve'] as $key=>$value)
		{
		     ...
		     * Send mail internal/external
		     */
		    admin_send_mail($db, null,         'disapproved', 'internal', $value, 1);
		    admin_send_mail($db, 'emailnotif', 'disapproved', 'external', $value, 1);
		}
	}
