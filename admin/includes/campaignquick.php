...
if(!empty($_GET["action"]) && $_GET["action"]=="start")
{
	$update = @mysql_query("UPDATE `tblCampaign` SET `running` = 'Y' WHERE `id` = '". $_GET['id'] ."' LIMIT 1");
        $update2= @mysql_query("Update tblMailQuick  set onhold=0 where campaignid = '". $_GET['id'] ."'");
...
if(!empty($_GET["action"]) && $_GET["action"]=="pause")
{
	...
	$update_mailquick = @mysql_query("update tblMailQuick set onhold=1 where campaignid = '". $_GET['id'] ."'");
...
if(!empty($_GET["action"]) && $_GET["action"]=="stop")
{
	$up     = @mysql_query("UPDATE `tblCampaignMails` SET `sent` = 'N', `readed` = 'N', `login` = 'N' WHERE `campaignid` = '" . $_GET['id'] . "'");
    	$upmq   = @mysql_query("Delete from tblMailQuick where campaignid='".$_GET['id']."'");
	$update = @mysql_query("UPDATE `tblCampaign` SET `quicktable`='N',`quickadding`='N',`finished` = 'N', `running` = 'N', `sent` = 0, `readed` = 0, `bounced` = 0, `login` = 0, `upgraded` = 0 WHERE `id` = '". $_GET['id'] ."' LIMIT 1");
	if(mysql_affected_rows() > 0){
...
if(!empty($_GET["action"]) && $_GET["action"]=="del")
{
	...
	$del_mailquick = @mysql_query("delete from tblMailQuick where campaignid = '". $_GET['id'] ."'");
...
{
	$dup_campaign = "INSERT INTO `tblCampaign` (`title`, `readyq`,`srvname`,`description`, `age_from`, `age_to`, `sex`, `looking`, `joinedfrom`, `joinedto`, 
		                         				...
		                         				`toseed`, `toevery`, `routed`, `date`, `country`) 
		                          select        `title`,`readyq`,`srvname`, `description`, `age_from`, `age_to`, `sex`, `looking`, `joinedfrom`, `joinedto`, 
		                         				`lastloginfrom`, `lastloginto`, `mailreceived`, `mailresponded`, `mailopened`, `loggedin`, `payed`, 
		                         				`cancelled`, `sendexternal`, `sendinternal`, `howmany`, `emailserver`,`interval`, `origin`,`originaccesslevel`,`emailstatus`,
		                         				`subjectextern`, `messageextern`, `subjectintern`, `messageintern`, `sendid`, `toscreenname`, 
		                         				`toseed`, `toevery`, `routed`, `date`,`country`
...
$col_array = mysql_fetch_array(mysql_query("SELECT `id`,`srvname`,`createdOn`,`title`,`description`,`finishedon`,`from`,`testCampaign`,
...
?>
<form name="form2" action="index.php?content=campaignquick" method="post">
<table style="vertical-align:top" align="center" cellpadding="0" cellspacing="20" border="0">	
	...
	<? 
		$qry = "SELECT * FROM `tblCampaign` where readyq='Y'";
 ...
 <?if($col_array['srvname'] == 'Y'){?> <td align="center" style="padding: 0px 5px 0px 5px; width:25px"><font class="tablecateg">Server</font></td> <? $columns++; } ?>
			...
			<tr onmouseover="this.style.backgroundColor='#CCCCCC'" onmouseout="this.style.backgroundColor='<?=$tdcolor ?>'" style="background-color:<?=$tdcolor ?>">
				 ...
				 <?if($col_array['srvname'] == 'Y'){?> <td align="center"><font class="tabletext"><?=$theaccount["srvname"]; ?></font></td> <?}?>
					...
					<?if($col_array['from'] == 'Y'){?> <td align="left">&nbsp;<font class="tabletext"><a href="index.php?content=viewuser&id=<?=$theaccount["sendid"]?>" target="_blank"><?=id_to_screenname($theaccount["sendid"]); ?></a></font></td> <?}?>
					    ...
					    <tr>
					      <td align="left" style="padding-left: 2px;">
					        ...
					        <?if($theaccount["running"] == "Y" AND $theaccount["finished"] == "N"){?>
					          <a href="javascript: if(confirm('Are you sure you want to pause this campaign?')){ document.location.href='index.php?content=campaignquick&action=pause&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Pause campaign" src="images/pause_button.jpg" width="16" height="16"></a>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					          <a href="javascript: if(confirm('Are you sure you want to stop this campaign?')){ document.location.href='index.php?content=campaignquick&action=stop&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Stop campaign" src="images/stop_button.jpg" width="16" height="16"></a>
					          <a href="javascript: if(confirm('Are you sure you want to delete this campaign?')){ document.location.href='index.php?content=campaignquick&action=del&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Delete campaign" src="images/delete_button.jpg" width="16" height="16"></a>
					          <a href="javascript: if(confirm('Are you sure you want to duplicate this campaign?')){ document.location.href='index.php?content=campaignquick&action=dup&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Duplicate campaign" src="images/dup_button.gif" width="16" height="16"></a>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					        ...
					        <?}elseif($theaccount["finished"] == "N"){?>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					          <a href="javascript: if(confirm('Are you sure you want to start this campaign?')){ document.location.href='index.php?content=campaignquick&action=start&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Start campaign" src="images/start_button.jpg" width="16" height="16"></a>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					          <a href="javascript: if(confirm('Are you sure you want to delete this campaign?')){ document.location.href='index.php?content=campaignquick&action=del&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Delete campaign" src="images/delete_button.jpg" width="16" height="16"></a>
					          <a href="javascript: if(confirm('Are you sure you want to duplicate this campaign?')){ document.location.href='index.php?content=campaignquick&action=dup&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Duplicate campaign" src="images/dup_button.gif" width="16" height="16"></a>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					        ...
					        <?} else {?>
					          ...
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					  	      <a href="javascript: if(confirm('Are you sure you want to delete this campaign?')){ document.location.href='index.php?content=campaignquick&action=del&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Delete campaign" src="images/delete_button.jpg" width="16" height="16"></a>
					  	      <a href="javascript: if(confirm('Are you sure you want to duplicate this campaign?')){ document.location.href='index.php?content=campaignquick&action=dup&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Duplicate campaign" src="images/dup_button.gif" width="16" height="16"></a>
					  	      <a href="javascript: if(confirm('Are you sure you want to re-send defered mails?')){ document.location.href='index.php?content=campaignquick&action=resend&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Re-send defered mails" src="images/resend_button.gif" width="16" height="16"></a>
					  	      <a href="javascript: document.location.href='index.php?content=editcampaign&id=<?=$theaccount["id"] ?>'; void(0);"><img border="0" alt="View & Edit Campaign" src="images/button_edit.gif" width="16" height="16"></a>
					        <?}
					          } elseif($theaccount["ready"] == "N"){?>
					  	      <a href="javascript: if(confirm('Are you sure you want to ADD MAILS to this campaign?')){ document.location.href='index.php?content=campaignquick&action=ready&id=<?=$theaccount["id"] ?>' }; void(0);" style="font-size: 10px; font-face: Verdana;"><u>ADD MAILS</u></a>
					  	      <a href="javascript: if(confirm('Are you sure you want to delete this campaign?')){ document.location.href='index.php?content=campaignquick&action=del&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Delete campaign" src="images/delete_button.jpg" width="16" height="16"></a>
					  	      <a href="javascript: if(confirm('Are you sure you want to duplicate this campaign?')){ document.location.href='index.php?content=campaignquick&action=dup&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Duplicate campaign" src="images/dup_button.gif" width="16" height="16"></a>
					  	      <img border="0" src="images/trans_button.gif" width="16" height="16">
					  	      <a href="javascript: document.location.href='index.php?content=editcampaign&id=<?=$theaccount["id"] ?>'; void(0);"><img border="0" alt="View & Edit Campaign" src="images/button_edit.gif" width="16" height="16"></a>
