...
{
	@mysql_query("DELETE FROM `tblTypeMails` WHERE `user_from` = '" . $_GET['del_user_from'] . "' AND `user_to` = '" . $_GET['del_user_to'] . "' AND `operator_id` = 0");
...
$nr = @mysql_fetch_array(mysql_query("SELECT COUNT(*) as count FROM `tblTypeMails` WHERE `user_id` = '".$_SESSION['sell_profile']."' AND `new` = 'Y' AND `folder` = '1'"));

$limit = $_GET['limit']>0?$_GET['limit']:10;
$page  = $_GET['page']>0?$_GET['page']:1;
$content = $_GET['content'];
$order = $_GET['order']!=''?$_GET['order'] . " " . $_GET['ttype']:"`date_sent` ASC, `user_id` ASC, `user_from` ASC";

while($page > ceil($nr['count']/$limit))$page--;

$profiles = mysql_query("SELECT `user_id`, count(*) as count FROM `tblTypeMails` WHERE `new` = 'Y' AND `folder` = '1' GROUP BY `user_id`");
...
$sql = "SELECT tTM.*, 
        ...
        WHERE `user_id` = '".$_SESSION['sell_profile']."' AND `new` = 'Y' AND `folder` = '1' 
        ORDER BY " . $order . " 
        LIMIT " . $limit . " 
        OFFSET " . (($page-1) * $limit);
    ...
    <td colspan="2">
      <table width="100%">
        ...
        <tr bgcolor="<? if(chat_mails_bgcolor($obj->user_to,$obj->user_from)){echo '#ABC9ED';}else{echo $color=$color=='#FFFFFF'?'#F2F2F2':'#FFFFFF';}?>" style="font-size: 13px; font-face: Verdana;">
	      ...
	      <td align="center"><a href="javascript: if (confirm('Are u sure you want to delete this set of messages?!')){document.location.href='index.php?content=chatmails&limit=<?=$limit?>&page=<?=$page?>&del_user_from=<?=$obj->user_from;?>&del_user_to=<?=$obj->user_to;?>&action=delete'}; void(0);">delete</a></td>
        </tr>
        <?}?>
        <?}else{?>
        <tr>
          <td align="center" height="3" bgcolor="#FFFFFF" colspan="9" style="font-face: Verdana; font-size:12px;">Select Profile First!</td>
        </tr>
        <?}?>
        <tr>
          <td height="3" bgcolor="#FFFFFF" colspan="9"></td>
        ...
        <tr class="tablecateg" style="background:url(pics/main/backgr_tabel_fade.jpg)">
		  <td style="padding-left: 10px;" align="left" colspan="9">
		    <a href="index.php?content=chatmails&limit=<?=$limit;?>&page=1">First</a>&nbsp;&nbsp;
		    <? if($page > 1){?> 
		    <a href="index.php?content=chatmails&limit=<?=$limit;?>&page=<?=($page-1);?>">Prev</a>&nbsp;&nbsp;
		    <? } else { ?>
		    Prev&nbsp;&nbsp
		    <? } ?>
		    &nbsp;&nbsp&nbsp;<?=$page;?> of <?=ceil($nr['count']/$limit);?>&nbsp&nbsp;&nbsp
		    <? if($page < ceil($nr['count']/$limit)){?> 
		    <a href="index.php?content=chatmails&limit=<?=$limit;?>&page=<?=($page+1);?>">Next</a>&nbsp;&nbsp;
		    <? } else { ?>
		    Next&nbsp;&nbsp
		    <? } ?>
	        &nbsp;&nbsp;<a href="index.php?content=chatmails&limit=<?=$limit;?>&page=<?=ceil($nr['count']/$limit);?>">Last</a>
		  </td>
        </tr>
		<tr>
		  <td colspan="9" height="3" bgcolor="#990000" width="100%"></td>
