<?
	if(!$_GET['id'] or $_GET['id'] <= 0){
		echo "<script>window.location='index.php?content=addmasswhispers'</script>";
		exit;
	}
	
	if(!empty($_POST["action"]) && $_POST["action"]=="edit")
	{
		@mysql_query("DELETE FROM `tblMassWhispersMails` WHERE `campaignid` = '" . (int) $_GET['id'] . "'");
		...
		$sql = "UPDATE `tblMassWhispers` SET `title` = '" . addslashes(trim($_POST['title'])) . "', 
	...
	$array_campaign = mysql_fetch_array(mysql_query("SELECT * FROM `tblMassWhispers` WHERE `id` = '" . $_GET['id'] . "'"));
?>
<form name="editform" method="post" action="index.php?content=editmasswhispers&id=<?=$_GET["id"]?>">
...
<table style="vertical-align:top" align="center" width="95%" height="95%" cellpadding="0" cellspacing="0" border="0">	
	...
	<!-- Page title line -->
	<tr>
			<td width="100%"><font class="pagetitle">Edit MassWhispers Campaign</font></td>
	</tr>
	<!-- Page content line -->
	<tr>
		<td height="10"></td>
				...
				<tr>
					...
					<?
						...
						for($j=0;$j<$nrq;$j++){
							...
							$select=($array_campaign["whisperid"]==$whisper["id"])?("selected='selected'"):("");
							echo "<option value='".$whisper["id"]."' ".$select.">".$whisper["whisper"]."</option>";
						}
					?>
					</select>
					</font></td>
