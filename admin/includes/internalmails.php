<?
	$query="select * from `tblMailerMachine` where `type`='internal'";
	$query=mysql_query($query);
	$nrmails=mysql_num_rows($query);
	if($_POST["action"]=="add"){
		$query="insert into `tblMailerMachine` (`for`,type,subject,message) values ('".$_POST["usefor"]."','internal','". addslashes($_POST["subject"])."','".addslashes($_POST["message"])."')";
		$query=mysql_query($query);
		$msg="Internal mail saved";
		echo "<script>document.location.href='index.php?content=internalmails&msg=".$msg."'</script>";
	}
	if($_POST["action"]=="del"){
		$qry="delete from `tblMailerMachine` where `id`='".$_POST["id"]."'";
		$qry=mysql_query($qry);
		$msg="Internal mail deleted";
		echo "<script>document.location.href='index.php?content=internalmails&msg=".$msg."'</script>";
	}
	if($_POST["action"]=="update"){
		...
		$qry=mysql_query($qry);
		$msg="Internal mail updated";
		echo "<script>document.location.href='index.php?content=internalmails&msg=".$msg."'</script>";
	}
?>
<form name="addform" method="post" action="index.php?content=internalmails">
...
<table style="vertical-align:top" align="center" width="95%" height="95%" cellpadding="0" cellspacing="0" border="0">	
	...
	<!-- Page title line -->
	<tr>
		<td width="100%"><font class="pagetitle">Internal Emails </font></td>
	</tr>
	<!-- Page content line -->
	<tr>
		<td height="10"></td>
	...
	<tr>
		<td><font class="tablename"><?=$nrmails ?> internal emails found</font></td>
	</tr>
	<tr>
		<td height="4"></td>
	</tr>
	...
	<tr>
		<td valign="top">
			...
			<tr onmouseover="this.style.backgroundColor='#CCCCCC'" onmouseout="this.style.backgroundColor='<?=$tdcolor ?>'" style="background-color:<?=$tdcolor ?>">
					...
					<option value="confirm_mail" <?=($theaccount["for"]=="confirm_mail")?("selected='selected'"):("") ?>>confirm_email</option>
					</select>
					</font></td>
	...
	<tr>
		<td><font class="tablename">Add internal email</font></td>
	</tr>
	<tr>
		<td height="4"></td>
	</tr>
