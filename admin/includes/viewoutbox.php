	...
	foreach($_POST['msg'] as $key=>$value)
		{
			...
			$to      = id_to_screenname(trim($email['user_to']));
			$message = htmlentities(strip_tags(trim(urldecode($_POST['messg']))));
			$savemail= 1;
			$redirect_to = "mem_mail.php?folder=inbox";
			
	 		if(!in_array($email['user_to'], $arr)){
				$error = box_sent_message($db, $to, $email['user_from'], $subject, $message, $savemail);
			}
...
<form name="editform" method="post" action="index.php?content=viewoutbox&id=<?=$_GET["id"] ?>">
<input type="hidden" name="action" value="" />
<input type="hidden" name="id" value="<?=$_GET["id"] ?>" />
<input type="hidden" name="mesajview" value="" />
<input type="hidden" name="mesajtbl" value="" />
<table style="vertical-align:top" align="center" width="95%" height="95%" cellpadding="0" cellspacing="0" border="0">	

	<!-- Page title line -->
	<tr>
		<td width="100%"><font class="pagetitle">Outbox for <?=id_to_screenname($_GET['id']);?></font></td>
	</tr>
	<tr>
		<td width="100%">
		  <input type="button" name="inbox" value="View Inbox" onClick="javascript:document.location.href='index.php?content=viewinbox&id=<?=$_GET['id']?>'">
		</td>
	</tr>
	<?php
	list($nrOfMails)  = mysql_fetch_row(mysql_query("(SELECT COUNT(*) AS HOWMANY FROM `tblMails` WHERE `user_id`='".$_GET['id']."' and `folder` = '2')"));
	list($nrOfMails2) = mysql_fetch_row(mysql_query("(SELECT COUNT(*) AS HOWMANY FROM `tblTypeMails` WHERE `user_id`='".$_GET['id']."' and `folder` = '2')"));
	$nrOfMails += $nrOfMails2;
	?>
	<tr>
		<td width="100%"><font class="pagetitle">Messages in <b>Outbox</b>:&nbsp;&nbsp;<?=$nrOfMails?$nrOfMails:0;?></font></td>
	</tr>
	<!-- Page content line -->
				...
				<td height="25" width="100%">
					...
					<table bgcolor="#FFFFFF" width="100%" cellpadding="1" cellspacing="1">
						<tr>
						   ...
						   <?php 																													
						   $qry_in=mysql_query("(SELECT `id`,`user_id`,`user_to`,`user_from`, `subject`, `message`, `folder`, `new`, `type`, `date_sent`, `operator_id`, 'tblMails' as tbl  FROM `tblMails` WHERE `user_id`='".$_GET['id']."' and `folder` = '2') UNION ALL (SELECT `id`,`user_id`,`user_to`,`user_from`, `subject`, `message`, `folder`, `new`, `type`, `date_sent`, `operator_id`, 'tblTypeMails' as tbl FROM `tblTypeMails` WHERE `user_id`='".$_GET['id']."' and `folder` = '2') ORDER BY `date_sent` DESC LIMIT ".($page-1)*$limit.",".$limit);
						   ...
						   <tr bgcolor="<?=$color=$color=='#FFFFFF'?'#F2F2F2':'#FFFFFF';?>" style="font-size: 13px; font-face: Verdana;">
                                 ...
                                 <td align="center"><span onclick="document.editform.actionn.value='vw'; document.editform.mesajview.value=<?=$row_in['id']?>; document.editform.mesajtbl.value='<?=$row_in['tbl']?>'; document.editform.submit();" style="cursor: hand; color: blue;"><?=$row_in["subject"] ?></span></td>
                            </tr> 
							<? }?>
					</table>
				</td>
	...
	if($_POST['actionn']=='vw')
		{
			...
			foreach($_POST['msg'] as $key=>$value)
				{
				$email = get_email($value, $_POST['mesajtbl']);
	?>
	<tr>
		<td>
			<table bgcolor="#CCCCCC" width="100%" cellpadding="0" cellspacing="1">
