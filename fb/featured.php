...
$featured = @$db->get_results("SELECT `id`, `screenname`, `country`, `city`, `typeloc`, `rating`, `votes`
                               FROM `tblUsers`
                               WHERE `featured` = 'Y'");
	  ...
	  $table[$i] .= "<tr>";
	    $table[$i] .= "<td style=\"text-align: center; vertical-align: middle;\">";
	      $table[$i] .= "<a href=\"" . $cfg['path']['url_site'] . "profileid.php?profile=" . $featured[$i]['id'] . "\"
	                        target=\"_self\">
	                        <img src=\"" . $cfg['path']['url_site'] . "showphoto.php?id=" . $featured[$i]['id'] . "&m=Y&t=r&p=1\" style=\"border: 1px solid #FFFFFF; width: 85px; height: 85px;\" alt=\"" . $featured[$i]['screenname'] . "\" />
	                     </a> ";
	    $table[$i] .= "<td>";
...
echo "<html><head>";
echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"" . $cfg['template']['url_template'] . "public/css/style.css\" /> ";
echo "</head><body>";
