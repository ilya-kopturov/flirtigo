	  ...
	  $table[$i] .= "<tr>";
	    $table[$i] .= "<td style=\"text-align: center; vertical-align: middle;\">";
	      $table[$i] .= "<a href=\"" . $profile_link . urlencode($featured[$i]['screenname']) . "\"
	                        target=\"_self\">
	                        <img src=\"" . $cfg['path']['url_site'] . "showphoto.php?id=" . $featured[$i]['id'] . "&m=Y&t=s&p=1\" style=\"border: 1px solid #FFFFFF; width: 90px; height: 90px;\" alt=\"" . $featured[$i]['screenname'] . "\" />
	                     </a> ";
	    $table[$i] .= "<td>";
...
if((int) $_SESSION['sess_id'] > 0){
  $html .= "<tr>";
    $html .= "<td style=\"text-align: right;\" class=\"featuredBoxLink\">
                [<a href=\"" . $cfg['path']['url_site'] . "mem_mostwanted.php\">more</a>]
              </td>";
  $html .= "</tr>";
}
