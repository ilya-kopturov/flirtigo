<?
	if($_POST["action"]=="approve")
	{
		...
		foreach($_POST['approve'] as $key=>$value)
		{
			...
			@mysql_query("UPDATE `tblVideos` SET `video_name` = '".mysql_real_escape_string($_POST['video_name'][$key])."', `video_description` = '".mysql_real_escape_string($_POST['video_description'][$key])."', `approved` = 'Y', `approvedby` = '1', `approveddate` = NOW() WHERE `id` = '".$value."'");
		    @mysql_query("UPDATE `tblUsers` SET `withvideo` = 'Y' WHERE `id` = '".$_POST['user_id'][$key]."' LIMIT 1");
		    
		    /**
		     * Send mail internal/external
		     */
		    admin_send_mail($db, null,         'video_approved', 'internal', $_POST['user_id'][$key], 1);
		    admin_send_mail($db, 'emailnotif', 'video_approved', 'external', $_POST['user_id'][$key], 1);
		}
	}
	if($_POST["action"]=="disapprove")
	{
		foreach($_POST['approve'] as $key=>$value)
		{
		     ...
		     * Send mail internal/external
		     */
		    admin_send_mail($db, null,         'video_disapproved', 'internal', $_POST['user_id'][$key], 1);
		    admin_send_mail($db, 'emailnotif', 'video_disapproved', 'external', $_POST['user_id'][$key], 1);
		}
	}
					          ...
					          <table bgcolor="#CCCCCC" width="500" style="font-size:13px; font-face:Verdana;" border="0" cellpadding="1" cellspacing="1">
					            <tr>
					             ...
					             <td width="250" align="center" valign="center">
					               <a href="javascript: window.open('video_player.php?vid=<?=$obj->id;?>','videowindow','resizable=yes,scrollbars=yes,width=500, height=450'); void(0);"><img src="<?=$cfg['path']['url_site'] . "videothumb.php?id=" . $obj->id;?>" border="1"></a>
					             </td>
					             <td width="230">
