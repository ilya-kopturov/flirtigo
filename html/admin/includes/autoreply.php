<?php
	...
	if($_POST["action"]=="add" && (int)$_GET["id"] >= 0){
		$qry="insert into `tblAutoWhisper` (`user_id`,`whisper_id`,`subject`,`message`,`minutes`) values('".(int)$_GET["id"]."','".(int)$_POST["whisper_new"]."','".addslashes($_POST["subject"])."','".addslashes($_POST["message"])."','".(int)$_POST["minutes"]."')";
		$qry=mysql_query($qry);
		$msg="Auto-reply was inserted into the database";
		echo "<script>document.location.href='index.php?content=autoreply&id=".(int)$_GET["id"]."&msg=".$msg."'</script>";
	}
	...
	if($_POST["action"]=="update" && (int)$_GET["id"] >= 0){
		echo $qry="update `tblAutoWhisper` set `whisper_id` = '".(int)$_POST["whisper".$_POST["id"]]."', `subject` = '".addslashes($_POST["subject".$_POST["id"]])."', `message` = '".addslashes($_POST["message".$_POST["id"]])."', `minutes` = '".(int)$_POST["minutes".$_POST["id"]]."' where `id` = '".(int)$_POST["id"]."'";
		$qry=mysql_query($qry);
		$msg="Auto-reply updated";
		echo "<script>document.location.href='index.php?content=autoreply&id=".$_GET["id"]."&msg=".$msg."'</script>";
	}
?> 
<form name="addform" method="post" action="index.php?content=autoreply<?=($_GET["id"]!="")?("&id=".$_GET["id"]):("") ?>">
	...
	<tr>
		<td valign="top">
			...
			<tr>
				...
				<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="100%" align="center">&nbsp;<font class="tabletext">Select Staff Accountuser: <select name="typeusr" onchange="document.location.href='index.php?content=autoreply&id='+document.addform.typeusr.value">
	...
	<tr>
		<td valign="top">
			...
			<tr onmouseover="this.style.backgroundColor='#CCCCCC'" onmouseout="this.style.backgroundColor='<?=$tdcolor ?>'" style="background-color:<?=$tdcolor ?>">
					...
					<b>Reply after:</b> <br /><input type="text" name="minutes<?=$reply["id"] ?>" value="<?=$reply['minutes']?>" /> minute(s)
					</font></td>
	...
	<tr>
		<td valign="top">
			...
			<tr onmouseover="this.style.backgroundColor='#f2f2f2'" onmouseout="this.style.backgroundColor='#cccccc'" style="background-color:#cccccc">
					...
					<td align="left" valign="top" width="65%"><input type="text" maxlength="35" name="minutes" value="" style="width: 350px;" /> <font class="tabletext">minute(s)</font></td>
