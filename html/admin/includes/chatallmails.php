...
$sql = "SELECT tTM.*, 
        ...
        ORDER BY " . $order
		. ($page > 0 ? "LIMIT " . abs(($page - 1) * $limit) . ",{$limit}" : '');
    ...
    <td colspan="2">
      <table width="100%">
        ...
        <tr bgcolor="<? if(chat_mails_bgcolor($obj->user_to,$obj->user_from)){echo '#ABC9ED';}else{echo $color=$color=='#FFFFFF'?'#F2F2F2':'#FFFFFF';}?>" style="font-size: 13px; font-face: Verdana;">
	      ...
	      <td align="center"><a href="javascript: if (confirm('Are u sure you want to delete this set of messages?!')){document.location.href='index.php?content=chatallmails&limit=<?=$limit?>&page=<?=$page?>&del_user_from=<?=$obj->user_from;?>&del_user_to=<?=$obj->user_to;?>&action=delete'}; void(0);">delete</a></td>
        </tr>
        <?}?>
        <tr>
          <td height="3" bgcolor="#FFFFFF" colspan="6"></td>
        ...
        <tr class="tablecateg" style="background:url(pics/main/backgr_tabel_fade.jpg)">
		  <td style="padding-left: 10px;" align="left" colspan="9">
		    <a href="index.php?content=chatallmails&limit=<?=$limit;?>&page=1">First</a>&nbsp;&nbsp;
		    <? if($page > 1){?> 
		    <a href="index.php?content=chatallmails&limit=<?=$limit;?>&page=<?=($page-1);?>">Prev</a>&nbsp;&nbsp;
		    <? } else { ?>
		    Prev&nbsp;&nbsp
		    <? } ?>
		    &nbsp;&nbsp&nbsp;<?=$page;?> of <?=ceil($nr['count']/$limit);?>&nbsp&nbsp;&nbsp
		    <? if($page < ceil($nr['count']/$limit)){?> 
		    <a href="index.php?content=chatallmails&limit=<?=$limit;?>&page=<?=($page+1);?>">Next</a>&nbsp;&nbsp;
		    <? } else { ?>
		    Next&nbsp;&nbsp
		    <? } ?>
	        &nbsp;&nbsp;<a href="index.php?content=chatallmails&limit=<?=$limit;?>&page=<?=ceil($nr['count']/$limit);?>">Last</a>
		  </td>
        </tr>
		<tr>
		  <td colspan="9" height="3" bgcolor="#990000" width="100%"></td>
