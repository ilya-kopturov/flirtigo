<?
	include("includes/createsite_class.php");
	if($_POST["action"]=="add"){
		if($_POST["typeusrs"]!=0 && $_POST["clients"]!=0){
			$aditional="";
			$and=($_POST["thequery"]!="")?(" and"):("");
			if($_POST["typeusrs"]!=1 || $_POST["clients"]!=1){
				if($_POST["typeusrs"]==1) $aditional.=$and." typeusr='1'";
				if($_POST["clients"]==1) $aditional.=$and." typeusr='0'";
			}
			$query=str_replace("\'","'","select * from tblUsers".$_POST["thequery"].$aditional.$_POST["thelimit"]);
			$query=mysql_query($query);
			$nrusers=mysql_num_rows($query);
			$nrcols=mysql_num_fields($query);
			$users="";
			$exporter="";
			for($i=0;$i<$nrusers;$i++){
				$theuser=mysql_fetch_array($query);
				for($j=0;$j<$nrcols;$j++){
					$exporter.=$theuser[$j].",";
				}
				$exporter.=($i!=$nrusers-1)?("|"):("");
				$users.=$theuser["Id"];
				$users.=($i!=$nrusers-1)?(","):("");
			}
			$upload=new uploadFiles($exporter,$_GET["id"],$_POST["ftpserver"],$_POST["ftpuser"],$_POST["ftppass"],$_POST["ftpfolder"]);
			$qry="update tblCreateSite set Users='".$users."', Url='".$_POST["ftpserver"].$_POST["ftpfolder"]."' where Id='".$_GET["id"]."'";
			$qry=mysql_query($qry);
		}
	}
?>
<form name="addform" method="post" action="index.php?content=editsitef">
...
<table style="vertical-align:top" align="center" width="95%" height="95%" cellpadding="0" cellspacing="0" border="0">	
	...
	<!-- Page title line -->
	<tr>
		<td width="100%"><font class="pagetitle">Create New Site -<font color="#990000"> Step 3 (final) </font></font></td>
	</tr>
	<!-- Page content line -->
	<tr>
		<td height="10"></td>
