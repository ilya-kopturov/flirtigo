...
function replace_before_send($replacestring, $to_array, $from_array)
{
	...
	$to_imagelink   = "<img src='" . $cfg['path']['url_site'] . "showphoto.php?id=" . $to_array['id'] . "&t=s&m=Y'>";
	$from_imagelink = "<img src='" . $cfg['path']['url_site'] . "showphoto.php?id=" . $from_array['id'] . "&t=s&m=Y'>";
	
	$to_videolink   = "<a href='" . $cfg['path']['url_site'] . "showvideo.php?id=" . $to_array['id'] . "&m=Y'>View Video</a>";
	$from_videolink = "<a href='" . $cfg['path']['url_site'] . "showvideo.php?id=" . $from_array['id'] . "&m=Y'>View Video</a>";
...
function admin_sent_message($db, $to, $from_id, $subject, $message, $savemail)
{
	...
	if($to_id['id'] > 0){
		...
		if(!$is_block){
			...
			{
				admin_send_mail($db,$email['email'],'new_message','external',$to_id['id'],$from_id);
			}
		}
	} else {
