...
{
	$update = @mysql_query("UPDATE `tblMassWhispers` SET `ready` = 'Y' WHERE `id` = '". $_GET['id'] ."' LIMIT 1");
	if(mysql_affected_rows() > 0){
		$msg = "ADD mails to MassWhispers Campaign has started!";
	} elseif(mysql_error()) {
		$msg = "MassWhispers Campaign WAS NOT Started! ERROR: " . mysql_error();
	} else {
		$msg = "ADD mails to MassWhispers Campaign has started!";
	}
}
...
{
	$update = @mysql_query("UPDATE `tblMassWhispers` SET `running` = 'Y' WHERE `id` = '". $_GET['id'] ."' LIMIT 1");
	if(mysql_affected_rows() > 0){
		$msg = "MassWhispers campaign Started Succesfully!";
	} else {
		$msg = "MassWhispers campaign WAS NOT Started!";
	}
}

if(!empty($_GET["action"]) && $_GET["action"]=="pause")
{
	$update = @mysql_query("UPDATE `tblMassWhispers` SET `running` = 'N' WHERE `id` = '". $_GET['id'] ."' LIMIT 1");
	if(mysql_affected_rows() > 0){
		$msg = "MassWhispers campaign Paused Succesfully!";
	} else {
		$msg = "MassWhispers campaign WAS NOT Paused!";
	}
}

if(!empty($_GET["action"]) && $_GET["action"]=="stop")
{
	$up     = @mysql_query("UPDATE `tblMassWhispersMails` SET `sent` = 'N', `readed` = 'N', `login` = 'N' WHERE `campaignid` = '" . $_GET['id'] . "'");
	$update = @mysql_query("UPDATE `tblMassWhispers` SET `finished` = 'N', `running` = 'N', `sent` = 0, `readed` = 0, `bounced` = 0, `login` = 0, `upgraded` = 0 WHERE `id` = '". $_GET['id'] ."' LIMIT 1");
	if(mysql_affected_rows() > 0){
		$msg = "MassWhispers Stopped Succesfully!";
	} else {
		$msg = "MassWhispers WAS NOT Stopped!";
	}
}

if(!empty($_GET["action"]) && $_GET["action"]=="del")
{
	$del_mails    = @mysql_query("DELETE FROM `tblMassWhispersMails` WHERE `campaignid` = '". $_GET['id'] ."'");
	$del_campaign = @mysql_query("DELETE FROM `tblMassWhispers` WHERE `id` = '". $_GET['id'] ."' LIMIT 1");
	
	if(mysql_affected_rows() > 0){
		$msg = "MassWhispers Deleted Succesfully!";
	} else {
		$msg = "MassWhispers WAS NOT Deleted!";
	}
}

if(!empty($_GET["action"]) && $_GET["action"]=="dup")
{
	$dup_campaign = "INSERT INTO `tblMassWhispers` (`title`, `description`, `age_from`, `age_to`, `sex`, `looking`, `joinedfrom`, `joinedto`, 
	...
	if(mysql_affected_rows() > 0){
		$msg = "MassWhispers Campaign Succesfully Duplicated!";
		@mysql_query("UPDATE `tblMassWhispers` SET `date` = NOW() WHERE `id` = " . mysql_insert_id());
	} else {
		$msg = "MassWhispers Campaign WAS NOT Duplicate!";
	}
}
?>
<form name="form2" action="index.php?content=masswhispers" method="post">
<table style="vertical-align:top" align="center" width="95%" cellpadding="0" cellspacing="20" border="0">	
	...
	<!-- Page title line -->
	<tr>
		<td width="100%"><font class="pagetitle">MassWhispers List </font></td>
	</tr>
	<tr>
	...
	<? 
		$qry = "SELECT * FROM `tblMassWhispers` ";
	...
	<tr>
		<td valign="top">
			...
			<tr height="25" style="background:url(pics/main/backgr_tabel_fade.jpg)">
				 <td align="center" style="width:50px"><font class="tablecateg">Id</font></td>
			...
			<tr onmouseover="this.style.backgroundColor='#CCCCCC'" onmouseout="this.style.backgroundColor='<?=$tdcolor ?>'" style="background-color:<?=$tdcolor ?>">
				<td align="left">&nbsp;<font class="tabletext"><?=$theaccount["id"]; ?></font></td>
					    ...
					    <tr>
					      <td align="left" style="padding-left: 2px;">
					        ...
					        <?if($theaccount["running"] == "Y" AND $theaccount["finished"] == "N"){?>
					          <a href="javascript: if(confirm('Are you sure you want to pause this MassWhispers MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=pause&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Pause MassWhispers campaign" src="images/pause_button.jpg" width="16" height="16"></a>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					          <a href="javascript: if(confirm('Are you sure you want to stop this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=stop&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Stop MassWhispers campaign" src="images/stop_button.jpg" width="16" height="16"></a>
					          <a href="javascript: if(confirm('Are you sure you want to delete this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=del&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Delete MassWhispers campaign" src="images/delete_button.jpg" width="16" height="16"></a>
					          <a href="javascript: if(confirm('Are you sure you want to duplicate this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=dup&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Duplicate MassWhispers campaign" src="images/dup_button.gif" width="16" height="16"></a>
					          <a href="javascript: document.location.href='index.php?content=editmasswhispers&id=<?=$theaccount["id"] ?>'; void(0);"><img border="0" alt="View & Edit MassWhispers campaign" src="images/button_edit.gif" width="16" height="16"></a>
					        <?}elseif($theaccount["finished"] == "N"){?>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					          <a href="javascript: if(confirm('Are you sure you want to start this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=start&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Start MassWhispers campaign" src="images/start_button.jpg" width="16" height="16"></a>
					          <img border="0" src="images/trans_button.gif" width="16" height="16">
					          <a href="javascript: if(confirm('Are you sure you want to delete this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=del&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Delete MassWhispers campaign" src="images/delete_button.jpg" width="16" height="16"></a>
					          <a href="javascript: if(confirm('Are you sure you want to duplicate this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=dup&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Duplicate MassWhispers campaign" src="images/dup_button.gif" width="16" height="16"></a>
					          <a href="javascript: document.location.href='index.php?content=editmasswhispers&id=<?=$theaccount["id"] ?>'; void(0);"><img border="0" alt="View & Edit MassWhispers campaign" src="images/button_edit.gif" width="16" height="16"></a>
					        ...
					        <?}
					  	      ...
					  	      <a href="javascript: if(confirm('Are you sure you want to ADD MAILS to this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=ready&id=<?=$theaccount["id"] ?>' }; void(0);" style="font-size: 10px; font-face: Verdana;"><u>ADD MAILS</u></a>
					  	      <a href="javascript: if(confirm('Are you sure you want to delete this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=del&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Delete MassWhispers campaign" src="images/delete_button.jpg" width="16" height="16"></a>
					  	      <a href="javascript: if(confirm('Are you sure you want to duplicate this MassWhispers campaign?')){ document.location.href='index.php?content=masswhispers&action=dup&id=<?=$theaccount["id"] ?>' }; void(0);"><img border="0" alt="Duplicate MassWhispers campaign" src="images/dup_button.gif" width="16" height="16"></a>
					  	      <a href="javascript: document.location.href='index.php?content=editmasswhispers&id=<?=$theaccount["id"] ?>'; void(0);"><img border="0" alt="View & Edit MassWhispers campaign" src="images/button_edit.gif" width="16" height="16"></a>
					        <?} else {?>
