...
class Image
{
	...
	function noresize($width, $height, $str, $font_size)
	{
	    if (!$this->image) {
	        return FALSE;
	    }
        ...
        if ($oldWidth > $width)
        {
                $shrinkage = $width/$oldWidth;
        }
        if($shrinkage !=1)
        {
                $newHeight = $shrinkage * $oldHeight;
                $newWidth = $width;
        }
        else
        {
        ...
        if($newHeight > $height)
        {
                $shrinkage = $height/$newHeight;
                $newWidth  = $shrinkage * $newWidth;
                $newHeight = $height;
        }
	    ...
	    $imageNew = @ImageCreateTrueColor($newWidth, $newHeight);    
	    imageCopyResampled($imageNew, $this->image, 0, 0, 0, 0, $newWidth, $newHeight, $newWidth, $newHeight);
