...
if(isset($_POST['submit'])){
	...
	{
		$curr_pass = @$db->query("SELECT `pass` FROM `tblUsers` WHERE `id` = '" . $_SESSION['sess_id'] . "' AND `pass` = '" . $_POST['current_password'] . "' LIMIT 1");
		if($curr_pass > 0){
				//@$db->query("UPDATE `tblUsers` SET `pass` = '" . $_POST['new_password'] ."' 
...
$smarty->display( $cfg['template']['dir_template'] . "login/" . "email.tpl" );
