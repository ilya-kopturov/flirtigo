...
if(($_POST['actiontype'] == "delete") && isset($_POST['mail_id'])) {
...
FROM `tblTypeMails` as t1
WHERE t1.reply_on_campaign_id IS NULL AND t1.`date_sent` > '{$daysago}' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1) = 1 AND t1.`new` = 'Y' AND t1.`folder` = 1 AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 1 AND
      t1.user_from NOT IN (
SELECT t1.user_from
FROM `tblTypeMails` as t1
WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 0))
...
(SELECT t1.*,
       ...
       'purple' as color, 3 as sort_col
FROM `tblTypeMails` as t1
WHERE t1.reply_on_campaign_id IS NULL AND t1.`new` = 'Y' AND t1.`folder` = 1 AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`user_to` = t1.`user_to` AND t2.`folder` = 1) = 1 AND
      t1.user_from NOT IN (
SELECT t1.user_from
FROM `tblTypeMails` as t1 WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 0
UNION
...
(SELECT t1.*,
       ...
       'blue' as color, 2 as sort_col
FROM `tblTypeMails` as t1
WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`date_sent` < '".$daysago."' AND t1.user_from NOT IN (
SELECT t1.user_from
FROM `tblTypeMails` as t1 WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 0
UNION
SELECT t1.user_from
	...
	if($obj->nrOfMails > 1) {
		$int = "
			...
			WHERE
				t1.`new` = 'Y'
				AND t1.`folder` = 1
				AND t1.`user_from` = '" . $obj->user_from . "'
				AND t1.`id` != " . $obj->id . "
			ORDER BY
				nrOfMailstoFake ASC,
				t1.`date_sent` ASC";
		$intquery = mysql_query($int);
		$obj->sub_mails = array();
		while ($objint = mysql_fetch_object($intquery)) {
			// get notes
			$objint->notes = get_notes($objint->user_from, $objint->user_to);
			
			$objint->nrOfMails = $obj->nrOfMails;
			if((int) $objint->nrOfMailstoFake == 1 OR $objint->mailNr == 1) {
				$objint->color		= 'purple';
				$objint->sort_col	= 3;
			} elseif($objint->date_sent < $daysago) {
				$objint->color		= 'blue';
				$objint->sort_col	= 2;
			} else {
				$objint->color		= 'green';
...
for ($i = 0; $i < count($data); $i++) {
	if($data[$i]->nrOfMails > 1) {
		$temp = $data[$i];
		$data[$i]->sub_mails[] = $temp;
		
		usort($data[$i]->sub_mails, 'cmp');
		
		$temp = $data[$i]->sub_mails[0];
		unset($data[$i]->sub_mails[0]);
		$temp->sub_mails = $data[$i]->sub_mails;
		$data[$i] = $temp;
	}
}
usort($data, 'cmp');
...
function setonchange(s){
	if(s.options[s.selectedIndex].value == "delete"){
		if (confirm('Are u sure you want to delete this set of messages?!')){
			document.forms["mailform"].action = "<?=$_SERVER['PHP_SELF']?>?<?=$_SERVER['QUERY_STRING']?>";
...
<iframe name="replytoall_iframe" id="replytoall_iframe" style="width: 1px; height: 1px;" frameborder="0"></iframe>
    ...
    <td colspan="2">
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr class="tablecateg" style="background:url(pics/main/backgr_tabel_fade.jpg)">
          ...
          <td width="2%" align="center" style="border: thin solid white;">&nbsp;</td>
		  <td width="10%" align="center" style="border: thin solid white;"><A href="index.php?content=chatallmails&limit=<?=$limit?>&page=<?=$page?>&order=user_from&ttype=<?if($_GET['order']=='user_from' and $_GET['ttype']=='asc') echo 'desc'; else echo 'asc'; ?>"><img src="images/sort/sort_off2.gif" border="0" align="absmiddle"></a>From</td>
          <td width="10%" align="center" style="border: thin solid white;"><A href="index.php?content=chatallmails&limit=<?=$limit?>&page=<?=$page?>&order=user_to&ttype=<?if($_GET['order']=='user_to' and $_GET['ttype']=='asc') echo 'desc'; else echo 'asc'; ?>"><img src="images/sort/sort_off2.gif" border="0" align="absmiddle"></a>To</td>
		  <td width="10%" align="center" style="border: thin solid white;">Country</td>
		  <td width="5%" align="center" style="border: thin solid white;"><A href="index.php?content=chatallmails&limit=<?=$limit?>&page=<?=$page?>&order=date_sent&ttype=<?if($_GET['order']=='date_sent' and $_GET['ttype']=='asc') echo 'desc'; else echo 'asc'; ?>"><img src="images/sort/sort_off2.gif" border="0" align="absmiddle"></a>Date</td>
		  ...
		  <td width="5%" align="center" style="border: thin solid white;">Reply</td>
		...
		<tr>
		  <td colspan="12" height="1" bgcolor="#990000" width="100%"></td>
		</tr>
        <tr>
          <td height="3" bgcolor="#FFFFFF" colspan="11"></td>
        </tr>
	    <tr>
	      <td colspan="11" style="padding: 0px; margin-bottom: 5px;"></td>
        ...
        <tr bgcolor="#ABC9ED" style="font-size: 13px; font-face: Verdana;">
		  ...
		  <td width="10%" style="padding-left: 5px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left">(<?php echo $usrType[idtofield($obj->user_from, "accesslevel")]; ?>) <a href="javascript: window.open('viewprofile.php?id=<?=$obj->user_from;?>','profilewindow','resizable=yes,scrollbars=yes,width=700, height=600'); void(0);"><?=id_to_screenname($obj->user_from);?></a></td>
          <td width="10%" style="padding-left: 5px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left"><a href="javascript: window.open('viewprofile.php?id=<?=$obj->user_id;?>','profilewindow','resizable=yes,scrollbars=yes,width=700, height=600'); void(0);"><?=id_to_screenname($obj->user_id);?></a></td>
		  ...
		  <td width="5%" align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;"><?= substr($obj->date_sent, 0, 10)?></td>
		  <td width="10%" style="padding-left: 5px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left"><?=substr($obj->subject,0,20);?>...</td>
		  ...
		  <td width="20%" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;">
        ...
        <tr id="reply_<?php echo $obj->id;?>" name="reply_<?php echo $obj->id;?>" style="display: none;" bgcolor="#ABC9ED">
          ...
          <td colspan="7">
            ...
            <input type="hidden" id="subjectReply<?php echo $obj->id;?>" value="<?php echo $subject;?>" name="subjectReply<?php echo $obj->id;?>" value="<?php echo $subject;?>" />
          </td>
          <td>
        ...
        <tr id="note_<?= $obj->id;?>" style="display: none;" bgcolor="#ABC9ED">
			<td colspan="3"></td>
			<td colspan="8">
            <textarea id="note_text_<?= $obj->id;?>" name="text" rows="4" style="width: 99%;"></textarea>
			</td>
          <td>
        ...
        <?php
        	if($obj->nrOfMails > 1) {
                ...
                foreach ($obj->sub_mails as $objint) {
        ...
        <tr bgcolor="<?php echo $color=$color=='#FFFFFF'?'#F2F2F2':'#FFFFFF';?>" style="font-size: 13px; font-face: Verdana;">
		  ...
		  <td width="10%" style="padding-left: 5px; border-left: 1px solid white; border-right: 1px solid white;" align="left"></td>
          <td width="10%" style="padding-left: 5px; border-left: 1px solid white; border-right: 1px solid white;" align="left"><a href="javascript: window.open('viewprofile.php?id=<?=$objint->user_id;?>','profilewindow','resizable=yes,scrollbars=yes,width=700, height=600'); void(0);"><?=id_to_screenname($objint->user_id);?></a></td>
		  ...
		  <td width="5%" align="center" style="border-left: 1px solid white; border-right: 1px solid white;"><?= substr($objint->date_sent, 0, 10) ?></td>
		  <td width="10%" style="padding-left: 5px; border-left: 1px solid white; border-right: 1px solid white;" align="left"><?=substr($objint->subject,0,20);?>...</td>
		  <td width="13%" style="padding-left: 5px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left"><?php echo checkStatus(id_to_screenname($obj->user_from)); echo " | "; if( (int) idtofield($obj->user_from, "accesslevel") > 0){echo "Active";}else{echo "Dead";} ?></td>
	      ...
	      <td width="20%" style="border-left: 1px solid white; border-right: 1px solid white;">
	      	<?php foreach ($objint->notes as $note) { ?>
		  		<div><?= htmlentities($note->text) ?> <b><?= htmlentities($note->admin_name) ?></b> <?= date('d.m.Y', strtotime($note->created)) ?></div>
		  	<?php } ?>
	      	<div id="view_note_<?= $objint->id;?>"><?= htmlentities($objint->note) ?> </div><a name="note<?= $objint->id;?>" id="note<?= $objint->id;?>" onclick="$('#note_<?= $objint->id;?>').toggle()" style="cursor: hand; color: blue;">edit notes</a>
	      </td>
	      <td width="6%" align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;">
        ...
        <tr id="reply_<?php echo $objint->id;?>" name="reply_<?php echo $objint->id;?>" style="display: none;" bgcolor="#ABC9ED">
          ...
          <td colspan="7">
            ...
            <input type="hidden" id="subjectReply<?php echo $objint->id;?>" value="<?php echo $subject;?>" name="subjectReply<?php echo $objint->id;?>" value="<?php echo $subject;?>" />
          </td>
          <td>
        ...
        <tr id="note_<?= $objint->id;?>" style="display: none;" bgcolor="#ABC9ED">
			<td colspan="3"></td>
			<td colspan="8">
            <textarea id="note_text_<?= $objint->id;?>" name="text" rows="4" style="width: 99%;"></textarea>
			</td>
          <td>
            <input type="button" name="save_note" value="Save" onclick="saveNote(document.getElementById('note_text_<?= $objint->id;?>').value, <?= $objint->user_from ?>, <?= $objint->user_to ?>, <?= $objint->id;?>)" />
          </td>
        </tr>
         <?php
                }
        ...
        <?}?>
        <tr>
          <td height="3" bgcolor="#FFFFFF" colspan="12"></td>
        </tr>
		<tr>
		  <td colspan="12" height="1" bgcolor="#990000" width="100%"></td>
		</tr>
        <tr class="tablecateg" style="background:url(pics/main/backgr_tabel_fade.jpg)">
		  <td style="padding-left: 10px;" align="left" colspan="12">
