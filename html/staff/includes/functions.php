...
/*function admin_send_mail($db,$email,$for,$type,$to_id,$from_id)
{
    ...
    $headers .= "From: HookupHotel.com <donotreply@hookuphotel.com>\r\n";
...
// changed by mar to use the user id instead
//	$to_id = @$db->get_row("SELECT `id`, `typeusr` FROM `tblUsers` WHERE `screenname` = '" . $to . "' LIMIT 1");
	$to_id = @$db->get_row("SELECT `id`, `typeusr` FROM `tblUsers` WHERE `id` = '" . $to . "' LIMIT 1");
	...
	if($to_id['id'] > 0){
		...
		if(!$is_block){
			...
			if($_SERVER['REMOTE_ADDR'] == "89.120.159.230"){
				echo "INSERT INTO `". $into_table ."` (`user_id`,`user_from`,`user_to`,`subject`,`message`,`date_sent`) VALUES ('" . $to_id['id'] . "','" . $from_id . "','" . $to_id['id'] . "','" . addslashes( $subject ) . "','" . addslashes( $message ) . "',NOW())";
				exit;
			}
...
function replytoall_sent_message($db, $to, $from_id, $subject, $message, $savemail)
{
	global $cfg;
	...
	$to_id = @$db->get_row("SELECT `id`, `typeusr` FROM `tblUsers` WHERE `disabled` = 'N' AND `id` = '" . $to . "' LIMIT 1");
	$into_table = $to_id['typeusr'] == 'Y'?'tblTypeMails':'tblMails';
...
function send_mail($to_email, $to_name, $subject, $message)
{
	...
	/*
	$mail->FromName = 'HookupHotel';
	$mail->From = 'alert@hookuphotel.com';
	$mail->Hostname = 'hookuphotel.com';
	$mail->Host     = 'hookuphotel.com';
	$mail->Sender   = 'alert@hookuphotel.com';
	$mail->Helo     = 'mail.hookuphotel.com';
	...
	*/
	...
	//$mail->Hostname = 'mail.hornybook.com';
	//$mail->Host     = 'mail.hornybook.com';
...
function bannedwords($db, $type = "B", $text = "", $client_domain_id = 1)
{
	$words = $db->get_results("SELECT `word` FROM `tblFilters` WHERE `type` != '" . $type . "' AND client_domain_id = " . (int)$client_domain_id);
...
function deletemails($user_from, $user_to, $operator_id)
{
	$smails = @mysql_query("SELECT * FROM `tblTypeMails` 
	                        WHERE `user_from` = '" . $user_from . "' AND `user_to` = '" . $user_to . "' AND `operator_id` = 0");
	
	while($obj = mysql_fetch_object($smails)){
		 mysql_query("INSERT INTO `tblTypeMailsDeleted` (`id`,`user_id`,`user_from`,`user_to`,`subject`,`message`,`folder`,`new`,`urgent`,`date_sent`,`operator_id`) 
		                                         VALUES ('" . $obj->id . "','" . $obj->user_id . "','" . $obj->user_from . "', 
		                                                 '" . $obj->user_to . "','" . addslashes($obj->subject) . "','" . addslashes($obj->message) . "', 
		                                                 '" . $obj->folder . "','" . $obj->new . "','" . $obj->urgent . "', 
		                                                 '" . $obj->date_sent . "','" . $operator_id . "')");
		
		@mysql_query("UPDATE `tblMails` SET `new` = 'N' WHERE `id_to_id` = '" . $obj->id . "'");
		
		@mysql_query("DELETE FROM `tblTypeMails` WHERE `id` = '" . $obj->id . "'");
	}
}
...
function idtofield($id, $field = "screenname"){
	global $db;
	
	return $db->get_var("SELECT `" . $field . "` FROM `tblUsers` WHERE `id` = '" . (int) $id . "'");
}

function checkStatus($screenname){
	...
	$sqlvero = "SELECT 1 
	            FROM `verolog` 
	            WHERE (`logdate` >= '" . $twomonths. "' AND `logdate` <= '" . $yesterday . "') AND 
	            `trn` = 'cancel' AND `usercode` = '" . $screenname . "'";


	if(@mysql_num_rows(mysql_query($sqlvero)) > 0){
		return "Cancelled";
	}
	
	
	$sqlccbill = "SELECT 1
				  FROM `ccbill_cancelation`
				  WHERE `cancelationDate` >= '" . $twomonths. "' AND `cancelationDate` <= '" . $yesterday . "' AND
				  		`subscription_id` = (SELECT `subscription_id` 
				  		                     FROM   `ccbill_all_ipn`
				  		                     WHERE  `username` = '" . addslashes($screenname) . "')";

	if(@mysql_num_rows(mysql_query($sqlccbill)) > 0){
		return "Cancelled"; 
	}
	
	
	$sql2000 = "SELECT 1 
	            FROM `tblPayments2000` 
	            WHERE (`date` >= '" . $twomonths. "' AND `date` <= '" . $yesterday . "') AND 
	                  `status` = 'del' AND `client_username` = '" . $screenname . "'";


	if(@mysql_num_rows(mysql_query($sql2000)) > 0){
		return "Cancelled";
	}
