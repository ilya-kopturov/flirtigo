    ...
    <td colspan="2">
      <table width="100%">
        ...
        <tr bgcolor="<? if(chat_mails_bgcolor($obj->user_to,$obj->user_from)){echo '#ABC9ED';}else{echo $color=$color=='#FFFFFF'?'#F2F2F2':'#FFFFFF';}?>" style="font-size: 13px; font-face: Verdana;">
	      ...
	      <td align="center"><? if((int) $obj->c_id == 0){?><a href="javascript: if (confirm('Are u sure you want to delete this set of messages?!')){document.location.href='index.php?content=chatmails&limit=<?=$limit?>&page=<?=$page?>&order=<?=$_GET['order'];?>&ttype=<?=$_GET['ttype'];?>&del_user_from=<?=$obj->user_from;?>&del_user_to=<?=$obj->user_to;?>&action=delete'}; void(0);">delete</a><?}?></td>
        </tr>
        <?}?>
        <?}else{?>
        <tr>
