...
$order = $_GET['order']!=''?$_GET['order'] . " " . $_GET['ttype']:"c_id DESC, nrofmails ASC, `date_sent` ASC, `user_id` ASC, `user_from` ASC";
    ...
    <td colspan="2">
      <table width="100%">
        ...
        <tr bgcolor="#ABC9ED" style="font-size: 13px; font-face: Verdana;">
	      ...
	      <td align="center"><? if((int) $obj->c_id == 0){?><a href="javascript: if (confirm('Are u sure you want to delete this set of messages?!')){document.location.href='index.php?content=chaturgentmails&limit=<?=$limit?>&page=<?=$page?>&order=<?=$_GET['order'];?>&ttype=<?=$_GET['ttype'];?>&del_user_from=<?=$obj->user_from;?>&del_user_to=<?=$obj->user_to;?>&action=delete'}; void(0);">delete</a><?}?></td>
        </tr>
        <?}?>
        <?}else{?>
        <tr>
        ...
        <tr class="tablecateg" style="background:url(pics/main/backgr_tabel_fade.jpg)">
		  <td style="padding-left: 10px;" align="left" colspan="9">
		    <a href="index.php?content=chaturgentmails&limit=<?=$limit;?>&page=1&order=<?=$_GET['order'];?>&ttype=<?=$_GET['ttype'];?>">First</a>&nbsp;&nbsp;
		    <? if($page > 1){?> 
		    <a href="index.php?content=chaturgentmails&limit=<?=$limit;?>&page=<?=($page-1);?>&order=<?=$_GET['order'];?>&ttype=<?=$_GET['ttype'];?>">Prev</a>&nbsp;&nbsp;
		    <? } else { ?>
		    Prev&nbsp;&nbsp
		    <? } ?>
		    &nbsp;&nbsp&nbsp;<?=$page;?> of <?=ceil($nr['count']/$limit);?>&nbsp&nbsp;&nbsp
		    <? if($page < ceil($nr['count']/$limit)){?> 
		    <a href="index.php?content=chaturgentmails&limit=<?=$limit;?>&page=<?=($page+1);?>&order=<?=$_GET['order'];?>&ttype=<?=$_GET['ttype'];?>">Next</a>&nbsp;&nbsp;
		    <? } else { ?>
		    Next&nbsp;&nbsp
		    <? } ?>
	        &nbsp;&nbsp;<a href="index.php?content=chaturgentmails&limit=<?=$limit;?>&page=<?=ceil($nr['count']/$limit);?>&order=<?=$_GET['order'];?>&ttype=<?=$_GET['ttype'];?>">Last</a>
		  </td>
        </tr>
		<tr>
		  <td colspan="9" height="3" bgcolor="#990000" width="100%"></td>
