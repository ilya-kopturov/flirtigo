...
if(isset($_GET['id']))
{
    ...
    $result=mysql_fetch_row($qresult);
        ...
        $_SESSION["sess_id"]          = $result[0];
        $_SESSION["sess_screenname"]  = $result[1];
        $_SESSION["sess_pass"]        = $result[2];
        $_SESSION["sess_accesslevel"] = $result[11];                                                                                                             }
}
...
$smarty->assign("pricetwo",$pricethree);
//////
...
if(isset($_POST['submit']))
{
 ...
 if($_POST['acctype']=='1')
  ...
  if($_POST['acctype']=='3')
	  {
    header_location('https://secure2.segpay.com/billing/poset.cgi?x-eticketid=116072:6915&memberID='.$_SESSION['sess_id'].'&x-auth-link=http%3A%2F%2Fwww.flirtigo.com%2F&x-auth-text=CLICK+HERE+TO+SIGNIN+TO+THE+MEMBERS+AREA!&x-decl-link=http%3A%2F%2Fwww.flirtigo.com%2Fupgrade%2Fsegpaydeclinedck.php&x-decl-text=CLICK+HERE+TO+TRY+OUR+ALTERNATIVE+PAYMENT+PROCESSOR&username='.$_SESSION['sess_screenname'].'&password='.$_SESSION['sess_pass']);
  }
 }
