...
if($_POST['actiontype'] == "delete")
{
	foreach($_POST['mail_id'] as $key => $value){
		//@mysql_query("DELETE FROM `tblTypeMails` WHERE `user_from` = '" . $value . "' AND `user_to` = '" . $key . "' AND `operator_id` = 0");
...
$result = mysql_query(
	...
	FROM
		`tblsettings`
	WHERE
		`key` = 'chat_days_for_urgent_mail'"
);
$row = mysql_fetch_assoc($result);

$daysago = date("Y-m-d H:i:s", mktime(0,0,0,date("m"),date("d") - (int)$row['value'],date("Y")));
...
$sqlall=mysql_query("select * from tblfakeaccess where fake=2 and operator=".$_SESSION['admin']);

$sqlpurples=mysql_query("select * from tblfakeaccess where fake=5 and operator=".$_SESSION['admin']);


if(mysql_num_rows($sqlall))
{
...
$profiles = mysql_query("SELECT tTM.`user_id`, count(tTM.`user_id`) as count,
                                         ...
                                         WHERE tTM.`new` = 'Y' AND tTM.`folder` = '1' and tTM.user_id = tfa.fake and tfa.operator=".$_SESSION['admin']."
                                         GROUP BY tTM.`user_id`
                                         ORDER BY joined DESC");
}
...
FROM `tblTypeMails` as t1
WHERE t1.reply_on_campaign_id IS NULL AND t1.`date_sent` > '{$daysago}' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1) = 1 AND t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 1 AND
      t1.user_from NOT IN (
SELECT t1.user_from
FROM `tblTypeMails` as t1
WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 0))
...
if(mysql_num_rows($sqlpurples)) $sql .="
UNION
(SELECT t1.*,
       (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1) as nrOfMails,
       (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`user_to` = t1.`user_to` AND t2.`folder` = 1) as nrOfMailstoFake,
...
FROM `tblTypeMails` as t1
WHERE t1.reply_on_campaign_id IS NULL AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1) > 1 AND t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 1 AND
      t1.user_from NOT IN (
SELECT t1.user_from
...
(SELECT t1.*,
       ...
       'purple' as color, 3 as sort_col
FROM `tblTypeMails` as t1
WHERE t1.reply_on_campaign_id IS NULL AND t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`user_to` = t1.`user_to` AND t2.`folder` = 1) = 1 AND
      t1.user_from NOT IN (
SELECT t1.user_from
FROM `tblTypeMails` as t1 WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 0
UNION
...
(SELECT t1.*,
       ...
       'blue' as color, 2 as sort_col
FROM `tblTypeMails` as t1
WHERE t1.reply_on_campaign_id IS NOT NULL AND t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) > 0
)
...
(SELECT t1.*,
       ...
       'blue' as color, 2 as sort_col
FROM `tblTypeMails` as t1
WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND t1.`date_sent` < '".$daysago."' AND t1.user_from NOT IN (
...
(SELECT t1.*,
       ...
       'green' as color, 4 as sort_col
FROM `tblTypeMails` as t1
WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND t1.user_from NOT IN (
SELECT t1.user_from
FROM `tblTypeMails` as t1 WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '" . $_SESSION['sell_profile'] . "' AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND t2.`date_sent` < t1.`date_sent`) = 0
UNION
SELECT t1.user_from
...
while($page > ceil($nr/$limit))$page--;			// wtf?
...
function get_notes($user_from, $user_to) {
	$sql = "
		SELECT
			n.*,
		...
		FROM
			tblchatnotes n
			LEFT JOIN tblAdmin a ON n.admin_id = a.id
		WHERE
			n.user_from = {$user_from}
			AND n.user_to = {$user_to}
		ORDER BY
			n.created ASC";
...
function setonchange(s){
	if(s.options[s.selectedIndex].value == "delete"){
		if (confirm('Are u sure you want to delete this set of messages?!')){
			document.forms["mailform"].action = "http://www.flirtigo.com<?=$_SERVER['PHP_SELF']?>?<?=$_SERVER['QUERY_STRING']?>";
			document.forms["mailform"].submit();
		}
	}else if(s.options[s.selectedIndex].value == "reply"){
		document.forms["mailform"].action = "http://www.flirtigo.com/staff/replyselected.php";
		document.forms["mailform"].submit();
	}
}
...
function saveNote(text, user_from, user_to, id) {
	document.forms["save_note_form"].text.value = text;
	document.forms["save_note_form"].user_from.value = user_from;
	document.forms["save_note_form"].user_to.value = user_to;
	document.forms["save_note_form"].submit();

	$('#note_' + id).hide();

	today = new Date();
	$('#view_note_' + id).before('<div>' + text + ' <b><?= $_SESSION['admin_name'] ?></b> ' + today.getDate() + '.' + (today.getMonth() + 1) + '.' + today.getFullYear() + '</div>');
...
<form method="post" action="save_note.php" target="save_note_iframe" name="save_note_form">
  <input type="hidden" name="user_from" value="" />
  <input type="hidden" name="user_to"   value=""   />
...
<iframe name="save_note_iframe" id="save_note_iframe" style="width: 1px; height: 1px;" frameborder="0"></iframe>
...
$reds =mysql_num_rows(mysql_query("SELECT t1.*,
       (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1) as nrOfMails,
              (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_f
                     (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE t2.`user_from` = t1.`user_to` AND t2.`user_to` = t1.`user_from` AND t2.`operator_id` != 0 AND t2
                            (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_f
                                   'red' as color, 1 as sort_col
                                   FROM `tblTypeMails` as t1
                                   WHERE t1.`new` = 'Y' AND t1.`folder` = 1 AND t1.`user_to` = '".$obj_p['user_id']."' AND (SELECT count( id ) FROM   `tblTypeMails` as t2
                                   WHERE ((t2.`new` = 'N' AND t2.`operator_id` != 0) OR (t2.`new` = 'Y')) AND t2.`user_from` = t1.`user_from` AND t2.`folder` = 1 AND
                                   t2.`date_sent` < t1.`date_sent`) = 0 AND (SELECT count( id ) FROM   `tblTypeMails` as t2 WHERE t2.`user_from` = t1.`user_to` AND
                                   t2.`user_to` = t1.`user_from` AND t2.`operator_id` != 0 AND t2.`folder` = 2) = 0"));
...
<input type="hidden" name="fakeUserId" value="<?php echo $_SESSION['sell_profile'];?>" />
  <tr>
    <td colspan="2">
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr class="tablecateg" style="background:url(pics/main/backgr_tabel_fade.jpg)">
		  ...
		  <td width="10%" align="center" style="border: thin solid white;"><A href="index.php?content=chatmails&limit=<?=$limit?>&page=<?=$page?>&order=user_from&ttype=<?if($_GET['order']=='user_from' and $_GET['ttype']=='asc') echo 'desc'; else echo 'asc'; ?>"><img src="images/sort/sort_off2.gif" border="0" align="absmiddle"></a>From</td>
          <td width="10%" align="center" style="border: thin solid white;"><A href="index.php?content=chatmails&limit=<?=$limit?>&page=<?=$page?>&order=user_to&ttype=<?if($_GET['order']=='user_to' and $_GET['ttype']=='asc') echo 'desc'; else echo 'asc'; ?>"><img src="images/sort/sort_off2.gif" border="0" align="absmiddle"></a>To</td>
		  <td width="10%" align="center" style="border: thin solid white;">Country</td>
		  <td width="5%" align="center" style="border: thin solid white;"><A href="index.php?content=chatmails&limit=<?=$limit?>&page=<?=$page?>&order=date_sent&ttype=<?if($_GET['order']=='date_sent' and $_GET['ttype']=='asc') echo 'desc'; else echo 'asc'; ?>"><img src="images/sort/sort_off2.gif" border="0" align="absmiddle"></a>Date</td>
		  <td width="10%" align="center" style="border: thin solid white;">Subject</td>
		  ...
		  <td width="20%" align="center" style="border: thin solid white;">Notes</td>
		...
		<? while($obj = mysql_fetch_object($query)){
			// get notes
			$obj->notes = get_notes($obj->user_from, $obj->user_to);
		...
		?>
		<tr bgcolor="#ABC9ED" style="font-size: 13px; font-face: Verdana;" onmouseover="this.style.backgroundColor='#FFF0F5'" onmouseout="this.style.backgroundColor='#ABC9ED'">
          <td align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;"><img width="10" height="10" src="images/color<?php echo $obj->color;?>.gif"></td>
		  <td style="padding-left: 5px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left">(<?php echo $usrType[idtofield($obj->user_from, "accesslevel")]; ?>) <a href="javascript: window.open('viewprofile.php?id=<?=$obj->user_from;?>','profilewindow','resizable=yes,scrollbars=yes,width=700, height=600'); void(0);"><?=id_to_screenname($obj->user_from);?></a></td>
          <td style="padding-left: 5px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left"><a href="javascript: window.open('viewprofile.php?id=<?=$obj->user_id;?>','profilewindow','resizable=yes,scrollbars=yes,width=700, height=600'); void(0);"><?=id_to_screenname($obj->user_id);?></a></td>
		  <td align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;"><? list($country) = mysql_fetch_array(mysql_query("SELECT `country` FROM `tblUsers` WHERE `id` = '" . $obj->user_from . "' LIMIT 1")); echo $countries[$country];?></td>
		  <td align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;"><?= substr($obj->date_sent, 0, 10) ?></td>
		  <td style="padding-left: 5px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left"><?=substr($obj->subject,0,20);?>...</td>
		  <td style="padding-left: 2px; border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;" align="left"><?php echo checkStatus(id_to_screenname($obj->user_from)); echo " | "; if( (int) idtofield($obj->user_from, "accesslevel") > 0){echo "Active";}else{echo "Dead";} ?></td>
		  <td align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;"><span name="checkreply<?php echo $obj->id;?>" id="checkreply<?php echo $obj->id;?>"><a name="reply<?php echo $obj->id;?>" id="reply<?php echo $obj->id;?>" onclick="showhide2('reply_<?php echo $obj->id;?>', <?php echo $obj->id;?>, 'reply');" style="cursor: hand; color: blue;">reply</a></span></td>
		  <td align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;"><a name="view<?php echo $obj->id;?>" id="view<?php echo $obj->id;?>" onclick="showhide2('view_<?php echo $obj->id;?>', <?php echo $obj->id;?>, 'view');" style="cursor: hand; color: blue;">view</a> (<?=$obj->nrofRepliestoUser;?>)</td>
	      <td style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;">
	      	<?php foreach ($obj->notes as $note) { ?>
		  		<div><?= htmlentities($note->text) ?> <b><?= htmlentities($note->admin_name) ?></b> <?= date('d.m.Y', strtotime($note->created)) ?></div>
		  	<?php } ?>
		  	<div id="view_note_<?= $obj->id;?>"><?= htmlentities($obj->note) ?> </div><a name="note<?= $obj->id;?>" id="note<?= $obj->id;?>" onclick="$('#note_<?= $obj->id;?>').toggle()" style="cursor: hand; color: blue;">edit notes</a>
	      </td>
	      <td width="6%" align="center" style="border-left: 1px solid white; border-top: 1px solid white; border-right: 1px solid white;">
	      		<input type="checkbox" name="mail_id[<?=$obj->user_from?>]" value="<?=$obj->id; ?>" />
	      </td>
        </tr>
          ...
          <td colspan="3" valign="top">
			...
			$sql = "
				SELECT
					ma.*
				FROM
					tblMailAttachments ma
					INNER JOIN tblMails m ON ma.email_id = m.id
				WHERE
					user_to = {$obj->user_to}
				    AND user_from = {$obj->user_from}
			";
			$result = mysql_query($sql);
			while ($attachment = mysql_fetch_object($result)) {
			?>
				<div>
					<img style="display: inline; vertical-align: middle;" width="16" src="/templates/site/dirtyflirting/login/images/dirtyflirting_mail<?= $attachment->type ?>.gif" />
					<a href="javascript:void(0)" onclick="openAttachment(<?= $attachment->id ?>)"><?= htmlentities($attachment->name) ?></a>
				</div>
			<?php } ?>
          	</div>
          </td>
        ...
        <tr id="note_<?= $obj->id;?>" style="display: none;" bgcolor="#ABC9ED">
			<td colspan="2"></td>
			<td colspan="8">
            <textarea id="note_text_<?= $obj->id;?>" name="text" rows="4" style="width: 99%;">
            	<?php foreach ($obj->notes as $note) { ?>
		  		<?= "<div>".htmlentities($note->text) ?> <?= "<b>".htmlentities($note->admin_name)."</b>" ?> <?= date('d.m.Y', strtotime($note->created))."</div>" ?>
		  		<?php } ?>
          ...
          <td>
            <input type="button" name="save_note" value="Save" onclick="saveNote(document.getElementById('note_text_<?= $obj->id;?>').value, <?php echo $obj->user_from ?>, <?php echo $obj->user_to ?>, <?= $obj->id;?>)" />
          </td>
        </tr>
        <?php
...
function openAttachment(id) {
	...
	$('#attachment_popup').jqm({
		ajax: 'view_attachment.php?id=' + id,
		modal: true,
		overlay: 25
	}).jqmShow();
}
