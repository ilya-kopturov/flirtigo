{* $Id: mail_reply.tpl 538 2008-06-13 15:53:10Z andi $ *}
...
<div class="generic_container clearfix">
	...
	<div style="float:left">
		<div>
			...
			<a href="{$cfg.path.url_site}profile/{screenname user_id=$email.user_from}">[view full profile]</a>
		</div>
		<div>
			<b>Subject</b>:
			<input class="textfield" type="text" name="subject" style="width:450px" value="{$email.subject|escape:'html'}">
		</div>
	...
	<div>
		...
		<div style="text-align:right;">
			<a href="javascript:;" onclick="if (confirm('Are you sure?')) $('#message_{$email.id}').load('{$cfg.path.url_site}ajax_read_message.php?id={$email.id}&f={$smarty.get.f}');">[cancel]</a>
...
$("#mail_form_{/literal}{$email.id}{literal}").validate({
	...
	submitHandler: function() {
		$("#mail_form_{/literal}{$email.id}{literal}").ajaxSubmit({
			url: {/literal}'{$cfg.path.url_site}ajax_message_reply.php?u=t&f={$smarty.get.f}'{literal},
			success: function() {},
			dataType: "script"
		});
	},
