	...
	<div id="whos_inside_block1">
		<div class="grey" id="whos_inside_block2">
			...
			<div class="container_text caption">
                        <a href="/join.php" style="text-decoration: none;">Join Now for Free</a>
			</div>
			...
			<div class="thumbs_container">
				...
				{section name="user" loop=$featured max=15}
					{if $featured[user].id}
						<div class="pic_user">
							<div class="thumb">
								<a href="/profile/{screenname user_id=$featured[user].id}">
								<img class="photo" src="/showphoto.php?id={$featured[user].id}&m=Y&t=s&p=1" alt="internet dating" /></a>
							</div>
						</div>
					{else}
				...
				{* Top Rated *}
						...
						{section name="user" loop=$toprated max=15 }
							{if $toprated[user].id}
								<div class="pic_user">
									<div class="thumb">
										<a href="/profile/{screenname user_id=$toprated[user].id}">
										<img	class="photo" src="/showphoto.php?id={$toprated[user].id}&m=Y&t=r&p=1" alt="adult social network" /></a>
									</div>
								</div>
							{else}
                                                <div class="pic_no_user"><img src="/templates/site/dirtyflirting/public/images/hornybook_bgpicturegrey_full.gif" alt="adult personals" /></div>
