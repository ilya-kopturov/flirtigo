...
if ($countryName == 'Great Britain (UK)') {
    ...
    if ($_POST[ptype] == 'ccbillcc') {
        $allowed = $pone . "," . $ptwo . "," . $pthree . "," . $pfour;
    }
...
if (isset($_POST['submit'])) {
    ...
    if ($_POST['ptype'] == 'ccbillcc') {
        if ($client_currency == 'USD') {
            ...
            if ($_POST['client_fname'] == '')
                $errorpayment .='First name field cannot be empty<br>';
            if ($_POST['client_lname'] == '')
                $errorpayment .='Last name field cannot be empty<br>';
            if ($_POST['client_address'] == '')
                $errorpayment .='Address field cannot be empty<br>';
            if ($_POST['client_zip'] == '')
                $errorpayment .='ZIP/Postal field cannot be empty<br>';
            if ($_POST['client_city'] == '')
                $errorpayment .='City field cannot be empty<br>';
            ...
            if ($_POST['acctype'] == 1) {
                header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8231&subscriptionTypeId=" . $tkid . "&currencyCode=840&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email']);
            };
            if ($_POST['acctype'] == 2) {
                header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8231&subscriptionTypeId=" . $tkid . "&currencyCode=840&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email']);
            };
...
//	syslog(LOG_INFO, var_export( , true));	
                ...
                if ($_POST['acctype'] == 1) {
                    header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8232&subscriptionTypeId=" . $tkid . "&currencyCode=978&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email']);
                };
                if ($_POST['acctype'] == 2) {
                    header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8232&subscriptionTypeId=" . $tkid . "&currencyCode=978&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email']);
                };
            ...
            if ($client_currency == 'GBP') {
                ...
                if ($_POST['acctype'] == 1) {
                    header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8230&subscriptionTypeId=" . $tkid . "&currencyCode=826&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email'] . "&country=GB");
                };
                if ($_POST['acctype'] == 2) {
                    header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8230&subscriptionTypeId=" . $tkid . "&currencyCode=826&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email'] . "&country=GB");
                };
...
//		header_location('https://secure2.segpay.com/billing/poset.cgi?x-eticketid='.$tkid.'&memberID='.$_SESSION['sess_id'].'&x-auth-link=http%3A%2F%2Fwww.flirtigo.com%2F&x-auth-text=CLICK+HERE+TO+SIGNIN+TO+THE+MEMBERS+AREA!&x-decl-link=http%3A%2F%2Fwww.flirtigo.com%2Fupgrade%2Fsegpaydeclinedcc.php&x-decl-text=CLICK+HERE+TO+TRY+OUR+ALTERNATIVE+PAYMENT+PROCESSOR&username='.$_SESSION['sess_screenname'].'&password='.$_SESSION['sess_pass']);
    ...
    if ($_POST['ptype'] == 'ccbillck') {
        ...
        if ($_POST['acctype'] == 1) {
            header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8233&subscriptionTypeId=" . $tkid . "&currencyCode=840&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email']);
        };
        if ($_POST['acctype'] == 2) {
            header("Location:https://bill.ccbill.com/jpost/billingCascade.cgi?clientAccnum=934240&clientSubacc=0004&language=English&cascadeId=8233&subscriptionTypeId=" . $tkid . "&currencyCode=840&username=" . $_SESSION['sess_screenname'] . "&password=" . $_SESSION['sess_pass'] . "&email=" . $_SESSION['sess_email']);
        };
